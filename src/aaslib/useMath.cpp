	...
	on(#OP, [](aas::Program& prog, std::size_t& pc){                                                              \
		...
		if(prog.stack.size() <= 1){                                                                               \
			...
			return 2;                                                                                             \
                                                                                                                  ...
                                                                                                                  \
		std::unique_ptr<aas::Data> _b = std::move(prog.stack.back());                                             \
		prog.stack.pop_back();                                                                                    \
                                                                                                                  \
		std::unique_ptr<aas::Data>& a = prog.stack.back();                                                        \
		std::unique_ptr<aas::Data>* b = &_b;                                                                      \
		if(b->get()->type == aas::DataType::REFERENCE){                                                           \
			b = &dynamic_cast<aas::Reference*>(b->get())->ref;                                                    \
		}                                                                                                         \
                                                                                                                  \
		aas::DataType dts[] = {DT};																			      \
		constexpr std::size_t dt_count = sizeof(dts) / sizeof(aas::DataType);								      \
		if(is_of_type(b->get()->type, dts, dt_count)){                                                            \
			prog.error = "\""#OP"\": Bad type of the first element of the stack: " + prog.src[pc]->strloc();      \
			return 3;                                                                                             \
		}                                                                                                         \
                                                                                                                  \
		if(a->type == aas::DataType::REFERENCE){                                                                  \
			aas::Reference* aref = dynamic_cast<aas::Reference*>(a.get());                                        \
			if(is_of_type(aref->ref->type, dts, dt_count)){                                                       \
				prog.error = "\""#OP"\": Bad type of the second element of the stack: " + prog.src[pc]->strloc(); \
				return 4;                                                                                         \
			}                                                                                                     \
			aref->ref = s_##OP(aref->ref.get(), b->get());                                                        \
		} else {                                                                                                  \
